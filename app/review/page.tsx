...
function ReviewContent() {
  ...
  const { data: reviewData, error: reviewError, isLoading: reviewLoading, mutate: mutateReview } = useSWR(
    ...
    async ([url]) => {
      // Fetch draft and generation in parallel
      const token = getAuthToken();
      const [draftRes, genRes] = await Promise.all([
        fetch(url, { headers: token ? { Authorization: `Bearer ${token}` } : {} }),
        fetch(`/api/v1/generations/${draftId}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} }),
      ]);
      if (!draftRes.ok) throw new Error("Draft not found");
      const draft = await draftRes.json();
      if (!genRes.ok) throw new Error("Generation not found");
      const generation = await genRes.json();
      return { draft: draft.data, generation: generation.data };
    },
    {
